...
(function () {
  ...
  function render() {
    ...
    list.forEach(p => {
      ...
      tr.innerHTML = `
        ...
        <td class="crm-row-actions">
          <button class="crm-icon-btn" data-measure="${p.id}" aria-label="Measure" title="Open in Measurements">📐</button>
      ...
      card.innerHTML = `
        ...
        <div class="quo-card-actions">
          <button class="crm-btn-ghost" data-measure="${p.id}">Measure</button>
    ...
    document.querySelectorAll("[data-measure]").forEach(btn => {
      btn.addEventListener("click", () => openInMeasurements(btn.dataset.measure));
    });
  }

  function openInMeasurements(id) {
    const p = projects.find(x => x.id === id);
    if (!p) return;

    const params = new URLSearchParams();
    if (p.customerId) params.set("customerId", p.customerId);
    params.set("projectName", p.name);
    if (p.locality) params.set("address", p.locality);

    window.open(`estimator/index.html?${params.toString()}`, "_blank");
  }
